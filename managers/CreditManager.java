...
public class CreditManager {
    ...
    public CompletableFuture<Boolean> purchaseCredits(Player player, int amount) {
        ...
        if (plugin.getVaultIntegration().withdraw(player, totalCost)) {
            return addCredits(player.getUniqueId(), amount).thenApply(success -> {
                if (success) {
                    Map<String, String> placeholders = messageManager.createPlaceholders(player);
                    placeholders.put("amount", String.valueOf(amount));
                    placeholders.put("cost", String.format("%.2f", totalCost));
                    messageManager.sendConfigMessage(player, "credits.purchased", placeholders);
                    
                    // Log transaction
                    logTransaction(player.getUniqueId(), amount, "PURCHASE", 
                                 "Purchased " + amount + " credits for $" + String.format("%.2f", totalCost));
                } else {
                    // Refund if credit addition failed
                    plugin.getVaultIntegration().deposit(player, totalCost);
